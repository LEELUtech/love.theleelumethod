...
// create-intent (checkout_viewed) — with advance-only Firestore write + analytics event
...
// ranks to prevent rollback (✅ removed abandoned)
...
export async function POST(req: NextRequest) {
	...
	try {
      ...
      name?: string;
		...
		const amount = typeof offering.price === "number" ? offering.price : 0; // cents
    ...
    const productName = offering.name ? String(offering.name) : null;
		...
		// Firestore: advance-only write (so repeated create-intent cannot rollback later steps)
		...
		try {
			await runTransaction(db, async (tx) => {
				...
				const patch: Record<string, unknown> = {
					...
					// stripe snapshot (✅ keep lowercase currency)
					...
					stripe_customer_id:
						...
						(intent.customer ? String(intent.customer) : null),
					...
					utm_first_campaign:
						cur.utm_first_campaign ?? utmFirstCampaign ?? null,
					...
					// ✅ IMPORTANT: store metadata as plain object (avoid Stripe.MetadataParam typing surprises)
			...
			console.error(
				"Failed to write checkout_viewed to Firestore (non-critical)",
				{
					requestId,
					intentId: intent.id,
					e,
		...
		try {
			const eventId = `ev_${Date.now()}_${Math.random().toString(16).slice(2)}`;
