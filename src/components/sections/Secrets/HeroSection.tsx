...
export default function SecretsHeroSection() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res?.success) throw new Error('Email was not sent. Please try again.');
  ...
  return (
    ...
    >
      ...
      <div className='container pt-0 pb-16 md:pt-12 md:pb-20 lg:pt-16 lg:pb-[112px] px-4'>
        <div className='relative flex flex-col items-center justify-center w-full text-center gap-4 md:gap-5 lg:gap-6'>
          ...
          <form onSubmit={onSubmit} className='w-full md:max-w-[360px] lg:max-w-[384px] px-4 md:px-0'>
            <input
              type='text'
              name='firstName'
            ...
            <Button
              ...
              trackingData={{
                ...
                cta_target_url: null,
                cta_location: 'secrets_hero',
              }}
            >
              DOWNLOAD THE FREE GUIDE
            </Button>

            {error ? <p className='mt-3 text-sm font-lato text-red-600'>{error}</p> : null}
            {success ? <p className='mt-3 text-sm font-lato text-green-700'>Sent! Check your inbox.</p> : null}
          </form>
        </div>
      </div>
    </section>
