...
export default function Button(props: ButtonProps) {
  ...
  const content = (
    <>
      {leftIcon ? (
        <span className={cx('mr-3 inline-flex shrink-0 items-center justify-center rounded-full', leftIconBg)}>
          <span className='relative inline-flex items-center justify-center'>{leftIcon}</span>
        </span>
      ) : null}
  ...
  return (
    <button type='button' disabled={disabled || loading} className={cls} onClick={handleClick} {...btnProps}>
      {content}
    </button>
  );
}
