...
export const sendWelcomeMessage: SendWelcomeMessage = async (memberId, memberName, tier) => {
  ...
  if (moderatorEmail) {
    const token = await getMemberToken(moderatorEmail);

    if (token) {
      const chatRes = await createChat(memberId, token.access_token, tier);

      if (chatRes) {
        await sendMessage(chatRes.chat_room.uuid, text, token.access_token);
      }
    }
  }
};
